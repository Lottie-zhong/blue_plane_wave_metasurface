...
SCRIPT_PATH = REPO_ROOT / "scripts" / "32_validate_apcd_k6_neighborhood_candidate_pool_geometry.py"
REPORT_PATH = REPO_ROOT / "reports" / "apcd_k6_neighborhood_candidate_pool_geometry_validation_note.md"


def test_can_read_neighborhood_candidate_pool_v1() -> None:
    candidates = read_candidate_pool_csv(CANDIDATE_POOL)

    assert len(candidates) == 24
    assert candidates[0]["candidate_id"] == "nhood_p1w_dx_01"


def test_neighborhood_validation_csv_columns_are_complete(tmp_path: Path) -> None:
    candidates = read_candidate_pool_csv(CANDIDATE_POOL)
    rows = validate_neighborhood_candidate_pool(candidates)
    output_csv = export_neighborhood_candidate_validation_csv(rows, tmp_path / "validation.csv")

    with output_csv.open("r", newline="", encoding="utf-8") as handle:
        reader = csv.DictReader(handle)
        loaded = list(reader)

    assert reader.fieldnames == NEIGHBORHOOD_VALIDATION_FIELDS
    assert len(loaded) == 24
    assert "source_reference" in loaded[0]


def test_neighborhood_family_distribution_is_correct() -> None:
    candidates = read_candidate_pool_csv(CANDIDATE_POOL)
    rows = validate_neighborhood_candidate_pool(candidates)
    summary = summarize_neighborhood_validation(rows)

    assert summary["total"] == 24
    assert summary["family_counts"] == {
        "bridge_dx_lhs": {"total": 4, "pass": 4, "fail": 0, "recommended": 4},
        "lhs_like_leakage_reduction": {"total": 10, "pass": 10, "fail": 0, "recommended": 10},
        "p1w_dx_neighborhood": {"total": 10, "pass": 10, "fail": 0, "recommended": 10},
    }


def test_all_neighborhood_parameters_in_bounds_and_rotations_fixed() -> None:
    candidates = read_candidate_pool_csv(CANDIDATE_POOL)
    rows = validate_neighborhood_candidate_pool(candidates)
...
def test_cli_dry_run_and_non_dry_run_do_not_generate_fsp(tmp_path: Path) -> None:
    ...
    assert "status=dry_run_neighborhood_geometry_validation_only_no_fdtd_no_lumapi_no_fsp_no_training" in dry.stdout
    assert "status=neighborhood_geometry_validation_only_no_fdtd_no_lumapi_no_fsp_no_training" in completed.stdout
...
def test_report_states_geometry_only_boundaries() -> None:
    ...
    assert "Geometry pass does not represent optical pass" in text
