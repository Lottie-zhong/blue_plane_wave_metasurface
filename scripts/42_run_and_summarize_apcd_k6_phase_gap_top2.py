...
TOP2_IDS = ["gap_bridge_03", "gap_lhs_leakred_06"]
DEFAULT_SELECTION_CSV = REPO_ROOT / "outputs/apcd_k6_active_learning/phase_gap_fdtd_selection_v1.csv"
DEFAULT_CONFIG_DIR = REPO_ROOT / "configs/apcd_k6_phase_state_candidates"
DEFAULT_RESULT_CSV = REPO_ROOT / "outputs/apcd_k6_active_learning/phase_gap_fdtd_results_v1.csv"

PHASE_GAP_RESULT_FIELDS = [
    "candidate_id",
    "candidate_family",
    "status",
    "target_conversion",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare and summarize APCD K=6 phase-gap top-2 FDTD rows.")
    parser.add_argument("--write-configs", action="store_true", help="Write config YAML for top-2 only.")
...
def top2_selection_rows(rows: Iterable[dict[str, str]], top2_ids: Sequence[str] = TOP2_IDS) -> list[dict[str, str]]:
    ...
    if missing:
        raise ValueError(f"missing top-2 candidate rows: {missing}")
...
def build_phase_gap_candidate_config(candidate: dict[str, str]) -> str:
  ...
  stage: 09_p16_apcd_k6_phase_gap_top2_real_eval
candidate:
  variant_id: {candidate_id}
  candidate_type: {candidate['candidate_family']}
  description: phase-gap 60-90 deg leakage-controlled selected candidate
  source_pool_csv: outputs/apcd_k6_active_learning/phase_gap_candidate_pool_v1.csv
  source_selection_csv: outputs/apcd_k6_active_learning/phase_gap_fdtd_selection_v1.csv
  notes: 09-P16 generated config for top-2 only; no config generated for backup gap_p2w_trim_03
boundary:
  no_k7: true
...
def write_top2_configs(selection_rows: Sequence[dict[str, str]], config_dir: str | Path) -> list[Path]:
    ...
    for row in top2_selection_rows(selection_rows):
        ...
        path.write_text(build_phase_gap_candidate_config(row), encoding="utf-8")
        written.append(path)
    return written
...
def result_row_from_values(
    *,
    ...
    return {
        ...
        "priority": "usable_60_90_phase_candidate" if usable else _priority(candidate_id, overall, inside_60_90, near_60),
        "notes": _notes(candidate_id, overall, inside_60_90, near_60),
    }


def result_row_from_results_csv(candidate_id: str, candidate_family: str, results_csv: str | Path) -> dict[str, object]:
    with Path(results_csv).open("r", newline="", encoding="utf-8") as handle:
        row = next(csv.DictReader(handle))
    return result_row_from_values(
        candidate_id=candidate_id,
        candidate_family=candidate_family,
        status=row["status"],
        target_conversion=float(row["target_conversion"]),
        opposite_spin_leakage=float(row["opposite_spin_leakage"]),
...
def write_phase_gap_result_csv(rows: Iterable[dict[str, object]], path: str | Path) -> Path:
    row_list = list(rows)
    output_path = Path(path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with output_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=PHASE_GAP_RESULT_FIELDS)
        writer.writeheader()
        writer.writerows({field: row.get(field, "") for field in PHASE_GAP_RESULT_FIELDS} for row in row_list)
    return output_path
...
def _priority(candidate_id: str, overall: bool, inside_60_90: bool, near_60: bool) -> str:
    ...
    if candidate_id == "gap_bridge_03":
        return "bridge_record_not_usable"
    return "record_not_usable"


def _notes(candidate_id: str, overall: bool, inside_60_90: bool, near_60: bool) -> str:
    parts = ["09-P16 real FDTD row; top-2 only"]
    if inside_60_90:
        parts.append("inside 60-90 deg")
    else:
        parts.append("outside 60-90 deg")
    if overall:
        parts.append("early pass")
    else:
        parts.append("not early pass")
    if near_60:
