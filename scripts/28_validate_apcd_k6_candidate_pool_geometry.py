...
DEFAULT_OUTPUT_CSV = "outputs/apcd_k6_active_learning/bounded_candidate_pool_v0_geometry_validation.csv"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate APCD K=6 candidate-pool geometry/gap sanity.")
...
def validate_candidate_pool_file(
    *,
    ...
    rows = validate_candidate_pool(candidates, minimum_gap_nm=minimum_gap_nm)
    export_candidate_validation_csv(rows, output_path)
    return output_path, rows, summarize_validation(rows)
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        print("status=dry_run_geometry_validation_plan_only_no_fdtd_no_lumapi_no_fsp_no_training")
        return 0

    written, rows, summary = validate_candidate_pool_file(
    ...
    print(f"anchor_status={summary['anchor_status']}")
    print(f"fail_reason_counts={summary['fail_reason_counts']}")
    print("status=geometry_validation_only_no_fdtd_no_lumapi_no_fsp_no_training")
    return 0
