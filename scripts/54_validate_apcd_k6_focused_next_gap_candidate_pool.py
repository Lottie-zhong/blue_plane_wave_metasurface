...
DEFAULT_SUMMARY = REPO_ROOT / "outputs/apcd_k6_active_learning/focused_next_gap_candidate_pool_v3_geometry_validation_summary.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate APCD K=6 focused next-gap candidate pool v3.")
...
def main() -> int:
    args = parse_args()
    rows = validate_focused_next_gap_candidate_pool(
    ...
    if not args.dry_run:
        write_csv_rows(rows, args.validation, FOCUSED_NEXT_GAP_VALIDATION_FIELDS)
        write_focused_validation_summary(args.summary, rows)
    return 0
