...
RESULTS_CSV = ACTIVE_DIR / "combined_phase_knob_p166_zero_cliff_recovery_results.csv"
DECISION_CSV = ACTIVE_DIR / "combined_phase_knob_p167_zero_cliff_recovery_decision.csv"
REPORT_MD = REPO_ROOT / "reports/combined_phase_knob_p166_zero_cliff_recovery_results.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Summarize existing real FDTD results for P165 zero-cliff recovery candidates."
    )
...
def main() -> int:
    ...
    print("status=stage09_zero_cliff_summary_built_no_fdtd_no_lumapi")
    return 0
...
def summarize_existing_result(plan: dict[str, str], result_csv: Path, raw: dict[str, str]) -> dict[str, object]:
    row = p164.summarize_result_row(plan["candidate_id"], "p165_plan", result_csv, raw)
    row["group"] = plan.get("group", "")
    row["family"] = plan.get("family", "")
    row["notes"] = "existing real FDTD result summarized; no FDTD run by P166"
...
def missing_result_row(plan: dict[str, str], result_csv: Path, note: str = "results.csv missing") -> dict[str, object]:
    return {
        ...
        "notes": f"{note}; run this candidate on the server before interpreting recovery status",
    }
...
def build_decision_rows(result_rows: Sequence[dict[str, object]]) -> list[dict[str, object]]:
    ...
    return [
        {
            "decision_key": "stage",
            "decision_value": "09-P167",
            "notes": "Stage 09 zero-cliff recovery summary only; no K=6 phase-ramp or steering claim.",
        },
        {
...
def write_report(path: Path, rows: Sequence[dict[str, object]], decisions: Sequence[dict[str, object]]) -> Path:
    ...
    lines = [
        ...
        "## Summary",
        "",
        f"- P165 candidates summarized: {len(rows)}",
