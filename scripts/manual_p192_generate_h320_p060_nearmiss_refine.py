from __future__ import annotations

from pathlib import Path
import csv
import copy
import re
import yaml

ROOT = Path(__file__).resolve().parents[1]

P191_PLAN = ROOT / "outputs/apcd_k6_active_learning/p191_h320_p060_leakage_recovery_plan.csv"
CONFIG_DIR = ROOT / "configs/apcd_k6_phase_state_candidates"

OUT_PLAN = ROOT / "outputs/apcd_k6_active_learning/p192_h320_p060_nearmiss_refine_plan.csv"
REPORT = ROOT / "reports/p192_h320_p060_nearmiss_refine_plan.md"

FIXED_H = 320

BASE_IDS = {
    "best": "p191_h320_p060_crot_m5_extra_crot_m2p5_p1W_m2_p2W_p2",
    "simple": "p191_h320_p060_crot_m5_extra_crot_m2p5",
    "c10_p2W4": "p191_h320_p060_crot_m10_p2W_p4",
}

# Very narrow refinement.
# Goal: reduce leakage from ~0.161 to <~0.145 while keeping phase inside 60-bin.
VARIANTS = [
    # Around best P191 near-miss, phase~89.67, ratio~5.40.
    ("best", "p1W_m1", "slightly increase width contrast without pushing phase too much", [
        {"type": "lateral", "target": "p1", "kind": "width", "delta": -1.0},
    ]),
    ("best", "p2W_p1", "slightly increase p2 width; may reduce leakage but phase risk", [
        {"type": "lateral", "target": "p2", "kind": "width", "delta": 1.0},
    ]),
    ("best", "p1W_m1_p2W_p1", "balanced extra width contrast", [
        {"type": "lateral", "target": "p1", "kind": "width", "delta": -1.0},
        {"type": "lateral", "target": "p2", "kind": "width", "delta": 1.0},
    ]),
    ("best", "p1W_m2_p2W_p1", "stronger p1 width reduction, mild p2 width up", [
        {"type": "lateral", "target": "p1", "kind": "width", "delta": -2.0},
        {"type": "lateral", "target": "p2", "kind": "width", "delta": 1.0},
    ]),
    ("best", "p1L_m1_p2L_p1", "tiny length contrast compensation", [
        {"type": "lateral", "target": "p1", "kind": "length", "delta": -1.0},
        {"type": "lateral", "target": "p2", "kind": "length", "delta": 1.0},
    ]),
    ("best", "p1L_m2_p2L_p2", "stronger length contrast compensation", [
        {"type": "lateral", "target": "p1", "kind": "length", "delta": -2.0},
        {"type": "lateral", "target": "p2", "kind": "length", "delta": 2.0},
    ]),
    ("best", "p2L_p1", "tiny p2 length up", [
        {"type": "lateral", "target": "p2", "kind": "length", "delta": 1.0},
    ]),
    ("best", "p2L_p2", "p2 length up", [
        {"type": "lateral", "target": "p2", "kind": "length", "delta": 2.0},
    ]),

    # Add small negative rotation to keep phase safely inside 60-bin while trying stronger width recovery.
    ("best", "extra_crot_m1_p2W_p2", "phase safety rotation plus p2 width recovery", [
        {"type": "rotation", "target": "all_core", "delta": -1.0},
        {"type": "lateral", "target": "p2", "kind": "width", "delta": 2.0},
    ]),
    ("best", "extra_crot_m1_p1W_m1_p2W_p2", "phase safety rotation plus width contrast", [
        {"type": "rotation", "target": "all_core", "delta": -1.0},
        {"type": "lateral", "target": "p1", "kind": "width", "delta": -1.0},
        {"type": "lateral", "target": "p2", "kind": "width", "delta": 2.0},
    ]),

    # Around simple P191 near-miss, phase~89.40, ratio~4.97.
    ("simple", "p1W_m3_p2W_p3", "same direction as best but slightly stronger", [
        {"type": "lateral", "target": "p1", "kind": "width", "delta": -3.0},
        {"type": "lateral", "target": "p2", "kind": "width", "delta": 3.0},
    ]),

    # Around c10_p2W4, phase~85.39, leakage~0.207. More phase margin, needs leakage reduction.
    ("c10_p2W4", "p1W_m2_p2W_p2", "more phase margin; try width contrast", [
        {"type": "lateral", "target": "p1", "kind": "width", "delta": -2.0},
        {"type": "lateral", "target": "p2", "kind": "width", "delta": 2.0},
    ]),
]


def sanitize(s: str) -> str:
    s = re.sub(r"[^A-Za-z0-9_]+", "_", s)
    return re.sub(r"_+", "_", s).strip("_")


def load_csv(path: Path) -> list[dict[str, str]]:
    with open(path, newline="", encoding="utf-8") as f:
        return list(csv.DictReader(f))


def dump_yaml(data: dict, path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with open(path, "w", encoding="utf-8") as f:
        yaml.safe_dump(data, f, sort_keys=False, allow_unicode=True)


def height_owners(data: dict) -> list[dict]:
    out = []

    def walk(obj):
        if isinstance(obj, dict):
            if "height_nm" in obj:
                out.append(obj)
            for v in obj.values():
                walk(v)
        elif isinstance(obj, list):
            for v in obj:
                walk(v)

    walk(data)
    return out


def set_all_heights(data: dict, h: int) -> None:
    owners = height_owners(data)
    if not owners:
        raise RuntimeError("No height_nm found.")
    for obj in owners:
        obj["height_nm"] = float(h)


def is_geometry_path(path: str) -> bool:
    p = path.lower()
    return any(t in p for t in [
        "geometry", "pillar", "nanopillar", "helper", "atom",
        "rect", "ellipse", "dimer", "meta", "p1", "p2"
    ])


def is_length_key(k: str, path: str) -> bool:
    s = k.lower()
    p = path.lower()
    return (
        "length" in s or "long_axis" in s or "major_axis" in s
        or s in {"l_nm", "len_nm"}
        or ("x_span" in s and is_geometry_path(p))
    )


def is_width_key(k: str, path: str) -> bool:
    s = k.lower()
    p = path.lower()
    return (
        "width" in s or "short_axis" in s or "minor_axis" in s
        or s in {"w_nm"}
        or ("y_span" in s and is_geometry_path(p))
    )


def is_rotation_key(k: str, path: str) -> bool:
    s = k.lower()
    return (
        "rotation" in s
        or s in {"rot_deg", "theta_deg", "angle_deg"}
        or s.endswith("_rot_deg")
    )


def pillar_tag(path: str) -> str:
    p = path.lower()

    if "helper" in p or "aux" in p:
        return "helper"

    p1_patterns = ["p1", "pillar1", "pillar_1", "nanopillar1", "nanopillar_1", "atom1", "atom_1", "rect1", "rect_1"]
    p2_patterns = ["p2", "pillar2", "pillar_2", "nanopillar2", "nanopillar_2", "atom2", "atom_2", "rect2", "rect_2"]

    if any(x in p for x in p1_patterns):
        return "p1"
    if any(x in p for x in p2_patterns):
        return "p2"
    return ""


def iter_numeric_fields(data: dict):
    def walk(obj, parent_path=""):
        if isinstance(obj, dict):
            for k, v in obj.items():
                path = f"{parent_path}.{k}" if parent_path else str(k)
                if isinstance(v, (int, float)) and not isinstance(v, bool):
                    yield parent_path, k, path, obj, float(v)
                yield from walk(v, path)
        elif isinstance(obj, list):
            for i, v in enumerate(obj):
                path = f"{parent_path}[{i}]"
                yield from walk(v, path)

    yield from walk(data)


def fallback_parent_groups(fields):
    groups = {}
    for f in fields:
        if f["tag"] == "helper":
            continue
        groups.setdefault(f["parent"], []).append(f)
    parents = sorted(groups.keys(), key=lambda x: x.lower())
    return [groups[p] for p in parents]


def lateral_fields(data: dict, kind: str):
    out = []
    for parent, key, path, obj, value in iter_numeric_fields(data):
        if not is_geometry_path(path):
            continue
        if kind == "length" and not is_length_key(key, path):
            continue
        if kind == "width" and not is_width_key(key, path):
            continue
        out.append({
            "parent": parent,
            "key": key,
            "path": path,
            "obj": obj,
            "value": value,
            "tag": pillar_tag(path),
        })
    return out


def rotation_fields(data: dict):
    out = []
    for parent, key, path, obj, value in iter_numeric_fields(data):
        if not is_geometry_path(path):
            continue
        if not is_rotation_key(key, path):
            continue
        out.append({
            "parent": parent,
            "key": key,
            "path": path,
            "obj": obj,
            "value": value,
            "tag": pillar_tag(path),
        })
    return out


def select_lateral_fields(data: dict, target: str, kind: str):
    fields = lateral_fields(data, kind=kind)

    if target == "all_core":
        core = [f for f in fields if f["tag"] != "helper"]
        return core if core else fields

    if target in {"p1", "p2"}:
        tagged = [f for f in fields if f["tag"] == target]
        if tagged:
            return tagged

        groups = fallback_parent_groups(fields)
        if target == "p1" and len(groups) >= 1:
            return groups[0]
        if target == "p2" and len(groups) >= 2:
            return groups[1]
        return []

    raise ValueError(f"Unknown lateral target={target}")


def select_rotation_fields(data: dict, target: str):
    fields = rotation_fields(data)

    if target == "all_core":
        core = [f for f in fields if f["tag"] != "helper"]
        return core if core else fields

    if target in {"p1", "p2"}:
        tagged = [f for f in fields if f["tag"] == target]
        if tagged:
            return tagged

        groups = fallback_parent_groups(fields)
        if target == "p1" and len(groups) >= 1:
            return groups[0]
        if target == "p2" and len(groups) >= 2:
            return groups[1]
        return []

    raise ValueError(f"Unknown rotation target={target}")


def apply_ops(data: dict, ops: list[dict]) -> list[str]:
    changed = []

    for op in ops:
        if op["type"] == "lateral":
            fields = select_lateral_fields(data, op["target"], op["kind"])
            if not fields:
                raise RuntimeError(f"No lateral fields selected for op={op}")

            for f in fields:
                old = float(f["obj"][f["key"]])
                new = old + float(op["delta"])

                if new <= 20 or new >= 300:
                    raise RuntimeError(f"Unphysical new lateral value {new} at {f['path']}")

                f["obj"][f["key"]] = float(round(new, 6))
                changed.append(f"{f['path']}:{old}->{new}")

        elif op["type"] == "rotation":
            fields = select_rotation_fields(data, op["target"])
            if not fields:
                raise RuntimeError(f"No rotation fields selected for op={op}")

            for f in fields:
                old = float(f["obj"][f["key"]])
                new = old + float(op["delta"])
                f["obj"][f["key"]] = float(round(new, 6))
                changed.append(f"{f['path']}:{old}->{new}")

        else:
            raise ValueError(f"Unknown op type={op['type']}")

    return changed


def update_result_dir(data: dict, candidate_id: str) -> None:
    data.setdefault("output", {})
    data["output"]["result_dir"] = (
        f"outputs/apcd_k6_metagrating_633nm/phase_state_candidates/{candidate_id}"
    )


def main() -> int:
    p191 = {r["candidate_id"]: r for r in load_csv(P191_PLAN)}
    rows = []

    for base_key, variant_id, purpose, ops in VARIANTS:
        base_id = BASE_IDS[base_key]

        if base_id not in p191:
            raise RuntimeError(f"Missing P191 base candidate in plan: {base_id}")

        base_config_rel = p191[base_id]["config_path"]
        base_config_path = ROOT / base_config_rel

        with open(base_config_path, "r", encoding="utf-8") as f:
            cfg = yaml.safe_load(f)

        cfg = copy.deepcopy(cfg)
        set_all_heights(cfg, FIXED_H)
        changed = apply_ops(cfg, ops)
        set_all_heights(cfg, FIXED_H)

        candidate_id = sanitize(f"p192_h320_p060_{base_key}_{variant_id}")
        config_path = CONFIG_DIR / f"{candidate_id}.yaml"

        cfg.setdefault("project", {})
        cfg["project"]["stage"] = "p192_h320_p060_nearmiss_refine"

        cfg.setdefault("boundary", {})
        cfg["boundary"]["fixed_height_nm"] = FIXED_H
        cfg["boundary"]["fabrication_aware_same_height"] = True
        cfg["boundary"]["not_k6_supercell"] = True
        cfg["boundary"]["not_steering_result"] = True
        cfg["boundary"]["not_micro_led_result"] = True
        cfg["boundary"]["p192_p060_nearmiss_refine"] = True

        cfg.setdefault("metadata", {})
        cfg["metadata"]["p192_base_key"] = base_key
        cfg["metadata"]["p192_base_candidate_id"] = base_id
        cfg["metadata"]["p192_variant_id"] = variant_id
        cfg["metadata"]["p192_purpose"] = purpose
        cfg["metadata"]["p192_fixed_height_nm"] = FIXED_H
        cfg["metadata"]["p192_ops"] = ops
        cfg["metadata"]["p192_changed_fields"] = changed[:50]

        update_result_dir(cfg, candidate_id)
        dump_yaml(cfg, config_path)

        rows.append({
            "candidate_id": candidate_id,
            "base_key": base_key,
            "variant_id": variant_id,
            "purpose": purpose,
            "base_candidate_id": base_id,
            "fixed_height_nm": FIXED_H,
            "config_path": str(config_path.relative_to(ROOT)).replace("\\", "/"),
            "changed_field_count": len(changed),
            "changed_fields_preview": " | ".join(changed[:6]),
        })

    OUT_PLAN.parent.mkdir(parents=True, exist_ok=True)
    with open(OUT_PLAN, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)

    REPORT.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# P192 h320 p060 near-miss refinement plan",
        "",
        "## Scope",
        "",
        "- Fixed-height h320 single-dimer candidates only.",
        "- Target: push P191 near-miss over early-pass threshold for 60 bin.",
        "- No height scan.",
        "- No K=6 supercell.",
        "- No steering claim.",
        "",
        f"generated_candidates: {len(rows)}",
        "",
        "## Candidate queue",
        "",
        "| base | variant | candidate | changed fields | purpose |",
        "|---|---|---|---:|---|",
    ]

    for r in rows:
        lines.append(
            f"| {r['base_key']} | {r['variant_id']} | `{r['candidate_id']}` | "
            f"{r['changed_field_count']} | {r['purpose']} |"
        )

    REPORT.write_text("\n".join(lines), encoding="utf-8")

    print(f"plan={OUT_PLAN}")
    print(f"report={REPORT}")
    print(f"generated_candidates={len(rows)}")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
