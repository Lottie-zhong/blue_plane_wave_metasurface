...
PLAN = ROOT / "outputs/apcd_k6_active_learning/p190_h320_m120_leakage_recovery_plan.csv"
OUT = ROOT / "outputs/apcd_k6_active_learning/p190_h320_m120_leakage_recovery_results.csv"
MD = ROOT / "reports/p190_h320_m120_leakage_recovery_results.md"
...
def main() -> int:
    ...
    for p in plan:
        ...
        opens_m120 = bool(early and nb == -120)
    ...
    hit_rows = [r for r in valid if r["opens_m120"] is True]
    ...
    lines = [
        ...
        f"- opens_m120: {len(hit_rows)}",
        f"- best_candidate: `{best['candidate_id'] if best else ''}`",
        f"- best_ratio: {best['conversion_to_leakage_ratio'] if best else ''}",
        f"- best_m120_candidate: `{best_hit['candidate_id'] if best_hit else ''}`",
        f"- best_m120_ratio: {best_hit['conversion_to_leakage_ratio'] if best_hit else ''}",
        "",
        "## Candidate results",
        "",
    ...
    for r in rows:
        lines.append(
            ...
            f"{r['early_pass']} | {r['opens_m120']} | `{r['candidate_id']}` |"
        )
